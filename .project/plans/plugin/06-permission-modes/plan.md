...
refs:
  ...
  - code://packages/agent-facio/src/session.ts - `agentOf`, which builds the run's policy from the mode
  ...
  - file:///github/facio/packages/agents/src/policy/modes.ts - `policyOf`, the mapping that moved into the core
  - file:///github/facio/packages/agents/src/policy/rules.ts - `DEFAULT_DECIDE`, which becomes the `auto` mode
  - file:///github/facio/packages/agents/src/types/model.ts#L14-L23 - `ReasoningEffort` and `ModelParams.reasoning`
  - file:///github/facio/packages/model-openai-compat/src/index.ts#L12 - `features.reasoning`, false until a host asks for it
---
...
- `rg "policy" packages/agent-facio/src` - the bridge passes `options.policy` straight into `createAgent`, so a mode setting has to compose with that or stand aside.
...
- `@ahpd/agent-facio`'s schema declares no `permissionMode` and no `effortLevel`, so the window draws neither control.
- `modelOf` builds the adapter with no `params` and no `features`, so a level has nowhere to go even if one were chosen.
- `Not found: a mode beyond papo's four anywhere in the harness - searched "permissionMode|plan|auto" in /github/facio/packages; the harness CLI's configuration is the only place the four are named.`
...
- **Layer responsibilities** - `/github/facio` (`@facio/agents`): `PermissionMode`, `policyOf` and the `inside`/`isEdit` seam · `@facio/papo`: unchanged behaviour, importing the mapping · `packages/agent-facio`: the two schema properties, the policy per turn, the model params · `docs/PLUGINS.md`: the two settings and what each value means.
- **Source-of-truth files** - `file:///github/facio/packages/agents/src/policy/modes.ts`, `code://packages/agent-facio/src/agent.ts`, `code://packages/agent-facio/src/session.ts`.
...
- **Watch out for:** the policy is built in `agentOf`, so a mode change takes effect on the next turn; and the facio-side move is uncommitted in `/github/facio`, so the bridge does not build without that working tree.
...
- [x] `pnpm typecheck` and `pnpm boundary` green: boundary green, typecheck clean apart from another session's in-progress `test/agent-facio-turn.test.ts`.
