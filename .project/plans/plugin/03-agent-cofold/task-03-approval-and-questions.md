...
refs:
  ...
  - file:///github/cofold/packages/agents/src/types/command.ts - `RunCommand`, the approve, deny, answer and steer a run accepts
  - file:///github/cofold/packages/agents/src/types/event.ts - `approval.requested`, `input.requested`, `run.paused` and their resolutions
  - file:///github/cofold/packages/agents/src/types/ask.ts - the questions and answers a request carries
  - file:///github/cofold/packages/agents/src/types/agent.ts#L36-L51 - `Policy`, whose `ask` is what raises an approval
...
- `UPDATE: packages/agent-cofold/src/session.ts` - the pending requests, `confirm`, `answer`, and the settlement.
- `UPDATE: packages/agent-cofold/src/mapping.ts` - the four pause and resolution events.
- `CREATE: test/agent-cofold-approval.test.ts` - the cases below.
...
- `test/agent-cofold-approval.test.ts`, with a stub model that proposes a destructive tool and a store in memory:
