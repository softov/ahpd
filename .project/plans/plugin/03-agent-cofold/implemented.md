...
Several of the items above were closed after the protocol and the code were read again; the list below supersedes the matching line above, and [deferred.md](deferred.md) is where the rest now points.
...
- An answer or a cancel that arrived while `start.resume` was still opening was dropped. Found on 2026-09-22 by the release workflow's suite: the replay announces the request the run is waiting on before `reopen` attaches the handle that takes commands, so `confirm` found the request, removed it and routed the decision to nothing, leaving the run `awaiting` for ever and the turn open. `route` and `stop` now wait on the same `opening` every turn waits on and send to whatever handle it left behind. `test/agent-cofold-store.test.ts` pins the window deterministically by queueing the confirmation as a microtask from inside the replayed `inputNeededSet`; with the fix reverted that test fails, the turn never ending.
