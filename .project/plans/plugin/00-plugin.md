...
- A plugin that wants to serve one host-owned URI scheme beside the filesystem store has to take the `resources` key over and wrap that store, so two such plugins cannot coexist; plan [08 - A plugin serves a host-owned URI scheme](08-resource-providers/plan.md).
