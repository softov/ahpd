...
refs:
  - code://packages/agent-facio/src/config.ts - `harnessConfig`, the file it reads and what it keeps
  - code://packages/agent-facio/src/agent.ts - `modelOf` and `resourceOf`, which consult it
  - code://packages/agent-facio/src/session.ts - the instructions fallback
  - file:///github/facio/packages/config/src/index.ts - `resolveConfig`, the harness's own layered reader
  - file:///github/facio/packages/papo/src/config.ts - `userConfigPath`, `providersOf` and `splitModel`, the wiring this mirrors
  - file:///github/facio/packages/model-openai-compat/src/index.ts - the adapter the provider builds
---

## Objective

`@ahpd/agent-facio` reads facio's own configuration at `$XDG_CONFIG_HOME/facio/config.json` and uses its `providers`, `model` and `instructions` as the defaults under the plugin's options and the session's settings, so a daemon whose plugin names no model and is given no token still runs on the provider the harness was already pointed at.

## Files

- `CREATE: packages/agent-facio/src/config.ts` - `harnessConfig`, `harnessConfigPath`, `splitModel` and their types.
- `UPDATE: packages/agent-facio/src/agent.ts` - `modelOf` and `resourceOf` take the harness config, the model reference selects a provider, and `facioAgent` reads it once for the defaults, `probe` and the advertised resource.
- `UPDATE: packages/agent-facio/src/session.ts` - the instructions fall back to the harness's, and the model factory is given the config.
- `UPDATE: packages/agent-facio/src/index.ts` - export the config helpers.
- `CREATE: test/agent-facio-config.test.ts` - the cases below.
