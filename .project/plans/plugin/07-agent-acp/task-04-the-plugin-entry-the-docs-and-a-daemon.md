...
- By hand, through the daemon's own argv (2026-09-22): `node packages/server/dist/main.js --plugin ./packages/agent-acp` served one real turn against `@deepseek-ai/dsh-acp`, run as the shipped `dsh --profile acp` profile, with a WebSocket client driving `initialize`, `createSession`, `subscribe` and a one-word `chat/turnStarted`. Provider `dsh`, `PONG` streamed, `chat/turnComplete`, no `chat/error`, and `permissionMode` in the session's config schema.
