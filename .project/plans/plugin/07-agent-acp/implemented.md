...
- **`@deepseek-ai/dsh-acp`, closed 2026-09-22.** The plan wanted the real daemon binary and this server, and both were run: `dsh --profile acp` is the shipped DSH ACP profile, and the daemon served a `PONG` turn through it. Nothing is left of the gap; what it cost is that DSH was already present in the harness's own `npx` checkout rather than installed here, the profile is zero-option, and `DSH_HOME` had to point somewhere writable because the sandbox's real one is not.
