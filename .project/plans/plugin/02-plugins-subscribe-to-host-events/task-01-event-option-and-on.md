...
refs:
  ...
  - code://packages/sdk/src/types/plugin.ts - `PluginHost`, which gains `on` and splits out `PluginContext`
...
`@ahpd/sdk` exports a `HostEvent` union, a `PluginContext` that is what a handler reads, and a `PluginHost.on(event, handler)` that records a listener into the plugin's contribution, which `foldHostOptions` merges into `HostOptions.events` in plugin order without disturbing handlers the base already carries.

## Files

- `CREATE: packages/sdk/src/types/events.ts` - `EventName`, the payload per event, `HostEvent`, `EventListener`, `HostHandlers`.
- `UPDATE: packages/sdk/src/types/plugin.ts` - `PluginContext`, `PluginHost extends PluginContext`, `on`, and `Contribution.events`.
...
5. In `types/plugin.ts`, split the read-only half out: `PluginContext` is `{ readonly path: string; readonly paths: string[]; readonly version: string; log(line: string): void }`, and `PluginHost extends PluginContext` adds the `register*` methods and `on`.
