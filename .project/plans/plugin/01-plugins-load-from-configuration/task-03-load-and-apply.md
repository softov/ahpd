...
refs:
  ...
  - code://packages/plugin/src/fold.ts - `foldHostOptions`, which this task calls with what `apply` produced
  - code://packages/plugin/src/types/plugin.ts - `Plugin`, `PluginHost` and `Contribution`, which the shape check enforces
...
- `UPDATE: packages/server/src/plugins.ts` - `readManifest`, `checkShape`, `makeHost`, `loadOne`, `loadPlugins`.
...
4. Write `makeHost(name, log)` returning a `PluginHost` and the `Contribution` it fills: `agent`/`agents` and `tool`/`tools` push onto the contribution, `port(key, value, when)` records `{ value, replace: when === 'replace' }` and pushes a problem immediately when the same plugin sets the same key twice, and `path`, `paths`, `version` and `log` are read from the options the daemon passed.
