...
`ahpd plugin` lists every plugin the configuration names, resolveable and not, with the path each spec resolves to and the name and title its manifest declares, and it does so without importing any plugin, so a person can see what a run would load and what it would run before any of it runs.
...
4. Print one line per spec as `<spec> -> <path> (<name>, <title>)`, using the module-independent manifest name, and `(no manifest)` where the spec is a file with no `package.json` above it, and `! <problem>` where it did not resolve.
...
- `test/plugin-list.test.ts`:
  ...
  - a plugin whose `index.ts` throws on import still lists, because nothing imported it.
