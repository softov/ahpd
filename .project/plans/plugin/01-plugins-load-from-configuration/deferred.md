...
- **Ports through `Start`.** A backend asking the host for file and shell access, which `@ahpd/agent-acp` needs.
  Built by [plan 07](../07-agent-acp/plan.md) task 03 and decision [acp-ports-come-through-start](../../../decisions/acp-ports-come-through-start.md): `Start` carries the host's `resources` and a host-owned `terminals` factory. It was a change to `Start` in `packages/sdk` and belonged to the agent plans in `ideas/agents-as-extensions.md`, not to the loader.
