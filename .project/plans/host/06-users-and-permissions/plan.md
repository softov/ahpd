...
| [05 - A client preference is the connection's, not the host's](task-05-a-client-preference-is-not-host-state.md) | done | 03 |
...
- **Open questions:**
  ...
  3. Whose preference does a backend's terminal use, when it has a session and no connection? - answered by the user, 2026-09-23: the host's default only. The session's owner was the honest answer and costs a creator on every session, persisted, which is a change to the session store this did not need.
...
- [x] `pnpm test` green: 70 files, 900 tests, with the directory, verification, gate matrix, the notification half, the per-connection shell and the unconfigured-daemon cases.
...
- [x] By hand: the real daemon with a user file, driven over a WebSocket. `listSessions` before signing in and after signing out answers `-32007`; a bad credential answers `-32007`; `ana` (admin) lists and writes; `sam`, on a `viewer` role of `read` alone, reads and is refused `-32009` on `resourceWrite`, `createTerminal` and `listSessions`; and the root snapshot carries the sign-in record with `required: true`.
- [x] Task 05 done, and the narrow `defaultShell` rule removed with it.
