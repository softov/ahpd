...
Source symbols keep their names: `FacioOptions`, `facioAgent`, `facioSession`, `facioTools`, `FacioAgent` and `FacioInput` are code identifiers rather than runtime identity, and renaming them is a separate edit with no bearing on what a client sees.
`.project/`'s `plugin/` plan and decision records keep their historical prose, which describes the work as it was done under the old name; only the broken `file:///github/cofold/...` path references are corrected there, because a path is a link and prose is a record.
