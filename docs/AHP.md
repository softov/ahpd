...
| `resolveSessionConfig` | the schema before a session exists | ✅ | The same schema a session reports, so a catalogue row is configurable before it is resumed. A backend advertises its own properties and a client draws what it is given; the Claude backend offers the same five approval modes VS Code's own Claude host does. `autoApprove` and `mode` are conventional keys a client dispatches whatever a host advertises, and are mapped onto that one axis on the way in This host contributes three properties of its own on top of the backend's - `isolation`, `branch` and `worktreeIncludeFiles` - when it was given a `worktrees` port and the directory is a git repository. They are host-owned: never passed to a backend, and absent entirely when there is no repository to make a worktree in, because a control with one value is one a client draws and nobody can move |
...
**75 of the 96 declared, across nine channels.** Grouped by channel; a group is
