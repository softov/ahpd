...
const fake = () => {
  ...
  const runtime: ComputerRuntime = {
    ...
    run: async (spec) => {
      calls.push(`run ${spec.name} ${spec.image} ${spec.cpus ?? '-'} ${spec.memory ?? '-'} ${spec.label}`);
    ...
    capabilities: () => ({ runtime: 'docker', actions: ['create', 'release', 'exec'], resources: ['status', 'capabilities'] }),
  };
  return { runtime, held, calls };
};

const options = { image: 'debian:bookworm-slim', cpus: '2', memory: '2g', max: 2 };
