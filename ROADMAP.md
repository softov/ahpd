...
Reported as an error today, and the decision is to stop reporting it that way: this host will synthesise the `resource` the state requires, the way the reference host already does for a harness that reports no better.

**What the retreat costs, measured against a client rather than argued.** VS Code turns `McpServerStatus.AuthRequired` into a sign-in inside the conversation: it reads `resource`, `oauthClient`, `authorization_servers`, `scopes_supported`, `requiredScopes` and `reason` off the state, renders a part the person can click, and on the click runs the workbench's own OAuth — dynamic client registration and all — then pushes the token back through AHP `authenticate`. Against `error` none of that is drawn. What a person gets is one line in a hidden Output channel, *Server 'x' failed: …*, at error severity. The `start` action sits on every server row whatever its state, so `startMcpServer` still reaches `reconnectMcpServer` and the CLI still runs its own sign-in — in a browser on the machine the daemon is on, which for a daemon somebody reached over a socket is the wrong machine.
...
**Both things this would have depended on are there.** The server's URL: `McpServerStatus` carries a `config` — "includes URL for HTTP/SSE servers" in the SDK's own words — which is `McpHttpServerConfig` or `McpSSEServerConfig`, so the canonical resource identifier comes off the same status this host already reads and no second reader of `.mcp.json` is needed. And somewhere to put a token a client hands back: `setMcpServers(servers)` re-declares a server at runtime, and both remote configs take `headers`, so an `Authorization` header is set the way the reference host sets codex's `http_headers`. Neither was checked when this entry was written, which is why it read as blocked. A stdio server still has no URL and stays an error, correctly.

**Suggestions.** (1) Taken: read the URL off `McpServerStatus.config`, discover `/.well-known/oauth-protected-resource` with a timeout, emit `authRequired` carrying what came back or bare `{ resource, resource_name }` when nothing did, and on `authenticate` re-declare the server through `setMcpServers` with the bearer in `headers`. (2) Emit `authRequired` and leave `reconnectMcpServer` as the only way through — a client draws the right control, and the sign-in still happens on the daemon's machine. Half the fix, and honest about which half; worth keeping as the fallback for a server whose metadata will not resolve. (3) Ask the SDK to report the authorization server as well, which would remove the discovery fetch.
