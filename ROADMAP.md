...
**The four commands not served.** Audited one at a time rather than labelled in a group, because "named refusal" turned out to be covering for two things that were not. The list skips `a`, `b`, `c`, `d` and `f`: `a` was the write half of `resource*`, `b` was `authenticate`, `c` was `sessionConfigCompletions`, `d` was `createResourceWatch`, `f` was the clock, and all five shipped. `c` was refused on the grounds that every key here is a small enum with nothing to look up - true until a capture of the reference host showed it marking its own `branch` row `enumDynamic`, which is the same key, on the same question, with the same answer: a repository has more branches than a picker holds. A letter is not reused any more than a number is.
