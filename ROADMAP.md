...
## A-02-05 — Nothing checks the *words* inside a payload, only its shape

`src/types/changes.ts` declared a changeset's status as `'computing' | 'complete' | 'error'` for as long as this host has served changesets. The protocol's `ChangesetStatus` is `computing | ready | error`. There is no `complete`, so every changeset ever served carried a status word no conformant client recognises — one reading it sees a changeset that never finishes computing.

**Why it survived a full conformance audit.** The audit counted the *actions* on each channel and the *fields* on each state, and this is neither: the field is present, its type is `string`, and its value is wrong. Typing the payloads against the package caught it only because a diff happened to need the field — `Bag` hid it before, and `OnWire<T>` catches a removed or misspelt *key* while a mistyped enum member passes anything the declaration allows. The audit's own TEST-001 would not have caught it either: it types the shape.

**What it costs today.** Unknown, which is the point. One was found by accident; the method that found it was accidental too. Every enum-valued field in every payload this host builds has the same exposure — `ToolCallStatus`, `TerminalLifecycleState`, `ChangesetOperationStatus`, `ChatOrigin.kind`, `SessionStatus`, `McpServerState.kind` — and several of those have already been wrong once for exactly this reason.

**Suggestions.** (1) Sweep every enum-valued field this host writes against the protocol's own declaration, mechanically: the package exports the enums as runtime objects, so the check is a script rather than a reading. (2) Narrow the local port types to the protocol's unions where one exists, so the next one is a compile error instead of a sweep — `ChangesetState.status` is now `'computing' | 'ready' | 'error'` by hand, and it could have been the protocol's own type. (3) Validate captured wire frames against `schema/state.schema.json` in CI, which catches vocabulary and shape together and needs no types at all — and is the only one of the three that keeps working when the protocol *adds* a value to an enum rather than changing one, since the other two check against a declaration this repository has already copied.
